...
rm(list = ls())
foldpath <- paste0("D:/workdir/23/17shap")
...
# ------------------ 3. 数据准备 ------------------
load("../../16imagediag/modelgroupdata.RData")
load("../../13corimage/sig_ImageFeatures_0.7.RData")
Imagefeatures <- sig_ImageFeatures

# Imagefeatures <- read.csv("../14wgcna_image/04.WGCNA_genes.csv")$symbol
# 
# Imagefeatures <- intersect(Imagefeatures,sig_ImageFeatures)

for (i in 1:2){
  # data[[i]]$group <- ifelse(data[[i]]$group == "PD",1,0)
  data[[i]] <- data[[i]][,-1]
  data[[i]] <- data[[i]][,c("group",Imagefeatures)]
}

dat <- rbind(data[[1]],data[[2]])
...
plot_rfe_performance <- function(rfe_data, alg_name) {
  ...
  # 保存图形
  ggsave(file.path(figures_dir, paste0(".RFE_Performance_", alg_name, ".png")), 
         combined_plot, width = 12, height = 5, dpi = 300)
  ggsave(file.path(figures_dir, paste0(".RFE_Performance_", alg_name, ".pdf")), 
...
plot_shap_analysis <- function(shp, optimal_data) {
  ...
  # 保存双格式
  ggsave(file.path(figures_dir, paste0(".SHAP_Global_Importance.png")), p_global, 
         width = 10, height = 8, dpi = 300)
  ggsave(file.path(figures_dir, paste0(".SHAP_Global_Importance.pdf")), p_global, 
  ...
  for (feature in top_features) {
    ...
    ggsave(file.path(figures_dir, paste0(".SHAP_Dependence_", feature, ".png")), 
           p_dep, width = 8, height = 6, dpi = 300)
    ggsave(file.path(figures_dir, paste0(".SHAP_Dependence_", feature, ".pdf")), 
...
plot_roc_curves <- function(pred_data) {
  ...
  save_roc_plot <- function(roc_list, colors, auc_values) {
    # 打开图形设备
    png_path <- file.path(figures_dir, paste0(".Algorithm_ROC_Comparison.png"))
    ...
    # PDF格式保存
    pdf_path <- file.path(figures_dir, paste0(".Algorithm_ROC_Comparison.pdf"))
