...
foldpath <- "D:/workdir/23/16imagediag"
...
# group <- genedata[,c(1,2)]
# tcgaimage <- merge(group,imagedata,by = "ID")
# save(tcgaimage,file = "tcgaimage.RData")

load("tcgaimage.RData")

load("../15hospgroup/data.RData")
data$group <- ifelse(data$group == "High",1,0)

index <- createDataPartition(data$group, p = 0.5, list = FALSE)
trainData <- data[index, ]
testData <- data[-index, ]

TrainData <- rbind(trainData,tcgaimage)
...
save(index,data,file = "modelgroupdata.RData")

load("../13corimage/sig_ImageFeatures_0.6.RData")
